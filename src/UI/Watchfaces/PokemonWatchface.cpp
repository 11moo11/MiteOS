...
void PokemonWatchface::draw() {
    uint32_t stepCount = int(ActivityManager::getStepCount() * .01);
    ...
    mDisplay.setCursor(PKM2_UI_X + 40, PKM2_UI_Y - 1);
    mDisplay.print(LVL_TXT);
    mDisplay.print(bat_level);
    ...
    mDisplay.setCursor(PKM1_UI_X + 36, PKM1_UI_Y - 1);
    mDisplay.print(LVL_TXT);
...
double PokemonWatchface::randomMinute(uint32_t d) {
    uint32_t seed = MiteOS::currentTime.Year;
    seed = seed * 12 + MiteOS::currentTime.Month;
    seed = seed * 31 + MiteOS::currentTime.Day;
    seed = seed * 24 + MiteOS::currentTime.Hour;
    seed = seed * 60 + MiteOS::currentTime.Minute;
    seed += d;
...
double PokemonWatchface::randomHour(uint32_t d) {
    uint32_t seed = MiteOS::currentTime.Year;
    seed = seed * 12 + MiteOS::currentTime.Month;
    seed = seed * 31 + MiteOS::currentTime.Day;
    seed = seed * 24 + MiteOS::currentTime.Hour;
...
double PokemonWatchface::randomDay(uint32_t d) {
    uint32_t seed = MiteOS::currentTime.Year;
    seed = seed * 12 + MiteOS::currentTime.Month;
    seed = seed * 31 + MiteOS::currentTime.Day;
    seed += d;
