...
#ifndef _MITEDISPLAY_BW_H_
#define _MITEDISPLAY_BW_H_
...
class MiteDisplay : public GxEPD2_GFX_BASE_CLASS
{
  public:
    GxEPD2_Type epd2;
    #if ENABLE_SCREENSHOTS == true
    bool pixelBuf[200 * 200];
    #endif
#if ENABLE_GxEPD2_GFX
    MiteDisplay(GxEPD2_Type epd2_instance) : GxEPD2_GFX_BASE_CLASS(epd2, GxEPD2_Type::WIDTH_VISIBLE, GxEPD2_Type::HEIGHT), epd2(epd2_instance)
#else
    MiteDisplay(GxEPD2_Type epd2_instance) : GxEPD2_GFX_BASE_CLASS(GxEPD2_Type::WIDTH_VISIBLE, GxEPD2_Type::HEIGHT), epd2(epd2_instance)
#endif
    {
    ...
    void drawPixel(int16_t x, int16_t y, uint16_t color)
    {
      ...
      if (color) {
        ...
        #if ENABLE_SCREENSHOTS == true
        pixelBuf[x * 200 + y] = true;
        #endif
      } else {
        _buffer[i] = (_buffer[i] & (0xFF ^ (1 << (7 - x % 8))));
        
        #if ENABLE_SCREENSHOTS == true
        pixelBuf[x * 200 + y] = false;
        #endif
      }
    }
    ...
    void screenshot() {
      ...
      Serial.print(booleanArrayToHexString(pixelBuf, 200 * 200));
      /*
      for(uint8_t x = 0; x < 200; x++) {
        for(uint8_t y = 0; y < 200; y++) {
          Serial.print(pixelBuf[x * 200 + y], HEX);
        }
      }
      */
      Serial.println();
